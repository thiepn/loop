...
const VERTEX_SOURCE = "#version 300 es\nin vec2 a_local;uniform vec2 u_resolution;uniform vec2 u_center;uniform vec2 u_half_vector;uniform float u_half_width;out vec2 v_local;void main(){float length_value=max(0.001,length(u_half_vector));vec2 direction=u_half_vector/length_value;vec2 normal=vec2(-direction.y,direction.x);vec2 position=u_center+u_half_vector*a_local.x+normal*u_half_width*a_local.y;vec2 zeroToOne=position/u_resolution;vec2 clip=zeroToOne*2.0-1.0;clip.y=-clip.y;gl_Position=vec4(clip,0.0,1.0);v_local=a_local;}";

const FRAGMENT_SOURCE = "#version 300 es\nprecision mediump float;in vec2 v_local;uniform vec4 u_color_a;uniform vec4 u_color_b;uniform float u_strength;uniform float u_pulse;out vec4 out_color;void main(){float transverse=exp(-v_local.y*v_local.y*3.8);float ends=smoothstep(1.0,0.42,abs(v_local.x));float core=transverse*ends;if(core<=0.002)discard;vec3 color=mix(u_color_a.rgb,u_color_b.rgb,v_local.x*0.5+0.5);color+=vec3(0.22,0.25,0.34)*u_pulse*core;float alpha=core*u_strength*(0.055+u_pulse*0.08);out_color=vec4(color,alpha);}";
