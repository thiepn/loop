...
const VERTEX_SOURCE = "#version 300 es\nin vec2 a0;uniform vec2 u4;uniform vec2 u15;uniform vec2 u16;out vec2 v0;void main(){vec2 position=u15+a0*u16;vec2 zeroToOne=position/u4;vec2 clip=zeroToOne*2.0-1.0;clip.y=-clip.y;gl_Position=vec4(clip,0.0,1.0);v0=a0;}";

const FRAGMENT_SOURCE = "#version 300 es\nprecision highp float;in vec2 v0;uniform float u18;uniform float u19;uniform vec4 u11;uniform vec4 u12;uniform float u20;uniform float u0;uniform float u17;uniform float u8;uniform float u13;uniform float u9;uniform float u5;uniform float u10;uniform float u6;uniform float u7;uniform float u14;uniform float u2;uniform float u1;uniform float u3;out vec4 o0;float hash21(vec2 p){p=fract(p*vec2(123.34,456.21));p+=dot(p,p+45.32+u20*9.7);return fract(p.x*p.y);}float fieldBoundary(float angle,float time){float waveA=sin(angle*(3.0+floor(u20*4.0))+time*0.37+u20*7.0);float waveB=sin(angle*(7.0+floor(u20*5.0))-time*0.23+u20*13.0);float rough=(waveA*0.62+waveB*0.38)*u0*u2;float toyDelta=0.0;if(u10>0.5 && u10<1.5)toyDelta=sin(angle*5.0+time*1.5)*u6*0.012;else if(u10>1.5 && u10<2.5)toyDelta=-u6*0.012;else if(u10>2.5 && u10<3.5)toyDelta=u6*0.018;else if(u10>3.5)toyDelta=sin(angle*2.0)*u6*0.016;return 0.985+rough+u13*0.024+toyDelta;}void main(){vec2 q=v0;float d=length(q);float angle=atan(q.y,q.x);float time=u14*0.001*u2;if(u18>0.5){float edge=1.0-smoothstep(0.62,1.0,d);if(edge<=0.001)discard;vec3 mixed=mix(u11.rgb,u12.rgb,0.5);float weave=0.5+0.5*sin((q.x+q.y)*15.0+time*1.2);float pulse=0.5+0.5*cos(d*20.0-time*1.5);vec3 color=u5>0.5?mix(mixed,vec3(0.76,0.78,0.96),0.42):mix(mixed,mix(u11.rgb,u12.rgb,weave),0.34);float alpha=edge*u9*(u5>0.5?0.08:0.11+pulse*0.035);o0=vec4(color,alpha);return;}float boundary=fieldBoundary(angle,time);float inside=1.0-smoothstep(boundary-0.04,boundary+0.012,d);float rim=smoothstep(boundary-0.11,boundary-0.02,d)*inside;if(inside<=0.001)discard;vec3 color=u11.rgb;float material=0.0;float alpha=u11.a*(0.34+inside*0.66);if(u19<0.5){float cloud=hash21(floor((q+1.0)*(6.0+u17*4.0)))*0.55+hash21(floor((q+1.0)*14.0+3.2))*0.45;float stars=step(0.94-u1*0.035,hash21(floor((q+1.0)*22.0)));material=cloud*0.45+stars*0.75;color=mix(color,vec3(0.32,0.58,1.0),0.18+material*0.22);alpha*=0.62+material*0.38;}else if(u19<1.5){float rings=0.5+0.5*cos(d*34.0-time*2.1);float gate=smoothstep(0.72,0.98,rings);material=gate*(0.35+u17*0.45);color=mix(color,vec3(0.45,0.92,1.0),material*0.35);alpha*=0.62+material*0.45;}else if(u19<2.5){float turbulence=sin(q.x*15.0+time*2.0+sin(q.y*11.0-time))*cos(q.y*13.0-time*1.5+q.x*4.0);material=0.5+0.5*turbulence;color=mix(color,vec3(1.0,0.2,0.06),material*0.48);alpha*=0.68+material*0.4;}else if(u19<3.5){float facets=abs(sin(angle*6.0+u20*5.0));float fractures=1.0-smoothstep(0.035,0.11,abs(sin(angle*11.0+d*18.0)));material=facets*0.35+fractures*0.65;color=mix(color,vec3(0.82,0.95,1.0),material*0.5);alpha*=0.65+material*0.38;}else{float threshold=smoothstep(-0.42,0.48,q.x+sin(q.y*5.0)*0.08);float bands=0.5+0.5*sin((q.x-q.y)*16.0+u20*8.0);material=threshold*0.58+bands*0.18;color=mix(vec3(0.03,0.2,0.18),vec3(0.24,0.98,0.72),threshold);color=mix(color,vec3(0.2,0.65,0.98),bands*0.16);alpha*=0.55+material*0.38;}color+=vec3(0.28,0.34,0.46)*rim*(0.09+u3*0.08);color+=vec3(0.24,0.28,0.36)*u7*0.055*inside;float selectedRing=u8*smoothstep(0.03,0.004,abs(d-min(1.07,boundary+0.045)));color=mix(color,vec3(0.94,0.96,1.0),selectedRing*0.78);alpha=max(alpha,selectedRing*0.7);alpha+=u13*rim*0.11;o0=vec4(max(color,vec3(0.0)),clamp(alpha*inside,0.0,0.48));}";
