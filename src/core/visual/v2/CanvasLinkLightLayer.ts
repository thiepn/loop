...
export class CanvasLinkLightLayer{
 ...
 public render(links:readonly RenderLink[],preferences:Readonly<VisualPreferences>,events:readonly RenderEventSample[],width:number,height:number,dpr:number){
  const boost=linkChoreographyBoost(events);
  for(const l of links)this.draw(l,preferences,events,width,height,dpr,1,boost);
 ...
 private draw(l:RenderLink,preferences:Readonly<VisualPreferences>,events:readonly RenderEventSample[],width:number,height:number,dpr:number,alpha:number,boost:number){
  ...
  this.ctx.save();this.ctx.beginPath();this.ctx.moveTo(pts[0]!.x,pts[0]!.y);for(let i=1;i<=max&&i<pts.length;i++){if(l.cross.fieldInfluence.frost>.34&&i%2===0)continue;this.ctx.lineTo(pts[i]!.x,pts[i]!.y);}this.ctx.strokeStyle=grad;this.ctx.lineCap='round';this.ctx.lineJoin='round';this.ctx.lineWidth=(l.selected?3.4:2.1)*dpr*(1+boost*.08);this.ctx.shadowBlur=(preferences.reduceBloom?2*dpr:8*dpr)*(1+boost*.18);this.ctx.shadowColor=renderColorCss(withAlpha(mixRenderColor(a,b,.5),(preferences.reduceBloom?.12:.3)*(1+boost*.2)));this.ctx.stroke();this.ctx.restore();
