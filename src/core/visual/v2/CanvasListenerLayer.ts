...
export class CanvasListenerLayer{
  ...
  public render(position:{x:number;y:number},state:Readonly<ListenerLightState>,playing:boolean,recording:boolean,preferences:Readonly<VisualPreferences>,timestampMs:number,width:number,height:number,dpr:number){
    const x=position.x*width,y=position.y*height,r=42*dpr*(1+state.energy*.08),ctx=this.ctx;
    ctx.save();
    const g=ctx.createRadialGradient(x,y,0,x,y,r*.95);g.addColorStop(0,renderColorCss(withAlpha(state.color,.82)));g.addColorStop(.45,renderColorCss(withAlpha(state.color,.12+state.energy*.14)));g.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=g;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();
